...
suggest_idx, good_shot = suggest_kinect(DistanceX, DistanceY, DistanceZ);
...
function NUMBER_VIE = FeedData(suggest_idx,good_shot)
    if good_shot == 1  
        NUMBER_VIE = 0;
    else
        %VIEList = [];
...
function [DistanceX, DistanceY, DistanceZ]  = get_kinect()
...
function [suggest_idx, good_shot] = suggest_kinect(DistanceX, DistanceY, DistanceZ)
    NumShots = 1;
    numColumns = 24;
