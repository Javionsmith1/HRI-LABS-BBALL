...
%COLLAPSE TIME
%always 285 time samples
NumShots = 10;
count = 1;
for i = 0:NumShots-1
    f_one_d = allFeatures(count:(i+1)*285,:);
    allFeatures_new(i+1,:) = reshape(f_one_d, 1, []);

    l_one_d = allLabels(count:(i+1)*285,:);
    allLabels_new(i+1) = l_one_d(1);

    count = (i+1)*285 + 1;
end
allLabels_new = allLabels_new';
%%
allFeatures = allFeatures_new;
allLabels = allLabels_new;

tableData_train_FeatNames = table(allFeatures,allLabels);
